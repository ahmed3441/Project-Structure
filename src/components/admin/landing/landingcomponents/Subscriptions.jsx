  ...
  const cards = [
    {
      ...
      points: [
        ...
        { icon: <GrCheckmark />, text: "Free domestic ATM withdrawls" },
      ],
    },
    {
      topText: "Smart",
      ...
      heading: "Everything in Stater, plus",
      points: [
        ...
        { icon: <GrCheckmark/>, text: "Discounter domestic " },
    ...
    },
    {
      ...
      points: [
        ...
        { icon: <GrCheckmark/>, text: "Free and international domestic ATM withdrawals  " },
      ],
    },
    {
      topText: "Black",
          ...
          <div className="flex justify-center">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-10">
              {cards.map((card, index) => (
                ...
                >
                  <div className="text-center text-lg font-bold text-[#1e293b] font-font-aspekta ">{card.topText}</div>
                  <img src={card.image} alt={card.topText} className="mx-auto mt-4 w-56" />
                  <div className="text-center mt-2 text-2xl font-bold text-[#1e293b] font-font-aspekta">{card.midText}</div>
                  {/* <button
                    className={`mt-4 px-4 py-2 rounded-md text-[15px] font-bold ${index === 1 ? 'bg-black text-white' : 'bg-blue-500 text-white'}`}
                  >
                    {card.buttonText}
                  </button> */}
                  <Button
                  ...
                  <div className=" mt-4 text-base font-medium text-[#1e293b]">{card.heading}</div>
                  <div className="mt-4">
                    {card.points.map((point, i) => (
                      <div key={i} className="flex items-center mt-2">
                        {/* <img src={point.icon} alt="Icon" className="w-6 h-6 mr-2" /> */}
                        <div className="mr-2 text-green-600 ">{point.icon}</div>
                        <div className="text-base font-normal text-[#1e293b]">{point.text}</div>
                      </div>
                    ))}
                  </div>
                </div>
